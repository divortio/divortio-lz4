...
describe('Lz4TestUtils (Shared Utilities)', () => {
    ...
    it('readU32/writeU32 should be symmetric', () => {
        ...
        Lz4TestUtils.writeU32(buf, val, 0);

        const readBack = Lz4TestUtils.readU32(buf, 0);
    ...
    it('readU16/writeU16 should be symmetric', () => {
        ...
        const val = 0xABCD;
        Lz4TestUtils.writeU16(buf, val, 0);

        const readBack = Lz4TestUtils.readU16(buf, 0);
        assert.strictEqual(readBack, val);
    ...
    it('ensureBuffer should handle Strings', () => {
        const buf = Lz4TestUtils.ensureBuffer("Hello");
    ...
    it('ensureBuffer should handle Objects (JSON)', () => {
        const obj = { a: 1 };
        const buf = Lz4TestUtils.ensureBuffer(obj);
    ...
    it('ensureBuffer should handle ArrayBuffer', () => {
        ...
        const buf = Lz4TestUtils.ensureBuffer(ab);
        assert.ok(buf instanceof Uint8Array);
    ...
    it('ensureBuffer should pass-through Uint8Array', () => {
        ...
        const output = Lz4TestUtils.ensureBuffer(input);
    ...
    it('getBlockId should map sizes to LZ4 constants', () => {
        ...
        assert.strictEqual(Lz4TestUtils.getBlockId(100), 4, "Small -> 64KB");
        assert.strictEqual(Lz4TestUtils.getBlockId(65536), 4, "64KB -> 64KB");

        assert.strictEqual(Lz4TestUtils.getBlockId(65537), 5, ">64KB -> 256KB");
        assert.strictEqual(Lz4TestUtils.getBlockId(262144), 5, "256KB -> 256KB");

        assert.strictEqual(Lz4TestUtils.getBlockId(262145), 6, ">256KB -> 1MB");

        assert.strictEqual(Lz4TestUtils.getBlockId(4194304), 7, "4MB -> 4MB");
        assert.strictEqual(Lz4TestUtils.getBlockId(undefined), 4, "Default -> 64KB");
    });
    ...
    it('createFrameHeader should produce valid structure', () => {
        ...
        const header = Lz4TestUtils.createFrameHeader(false, false, 7);

        assert.strictEqual(header.length, 7);

        // Magic Number
        const magic = Lz4TestUtils.readU32(header, 0);
    ...
    it('createFrameHeader should set flags correctly', () => {
        ...
        const header = Lz4TestUtils.createFrameHeader(true, true, 4);
